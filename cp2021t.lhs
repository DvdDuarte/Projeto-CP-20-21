...
O primeiro problema tem como tema um ExpAr.
Para a primeira alinea vimos o comportamento da função inExpAr e com base nisso definimos o outExpAr.
...
outExpAr (Bin x y z) = i2 (i2 (i1 (x,(y,z))))
...
Para fazer esta alinea recorremos ao seguinte diagrama:
...
\xymatrix@@C=2cm{
    |ExpAr|
           \ar[d]_-{|cataNat g|}
&
    |1 + A +(BinUp >< (ExpAr A >< ExpAr A)) +(UnOp >< ExpAr A)|
           \ar[d]^{|id + id +(id >< (cataNat g >< cata g))+(id >< cata g)|}
           \ar[l]_-{|inNat|}
\\
     |A|
&
...
\begin{code}
g_eval_exp escalar = either g1 g2
     where g1=(const escalar)
           g2=either id g3 
           g3= either g4 g5
           g4(operacao,(d1,d2))
            | (operacao == Product) = d1*d2
            | (operacao == Sum) = d1+d2
           g5(operacao,d1)
            | (operacao== Negate) = -d1
            | (operacao== E )= expd d1
...
---
clean :: ExpAr a  -> Either () (Either a (Either (BinOp, (ExpAr a, ExpAr a)) (UnOp, ExpAr a)))
clean X = i1 ()
clean (N n) =  i2 (i1 n)
clean (Bin x y z) = i2 (i2 (i1 (x,(y,z))))
clean (Un op a) = i2 (i2 (i2 (op,a)))


gopt e = either c (either n (either bop uop)) 
       where c = const e 
             n x = x
             bop (Sum, (0,d2)) = d2
             bop (Sum,(d1,0)) = d1 
             ...
             bop (Product, (1,d2)) = d2
             bop (Product,(d1,1)) = d1  
             bop (Product,(d1,d2)) = d1*d2
             uop (Negate,d1) = -d1
             uop (E,d1) = expd d1
            
        
\end{code}
Para resolver este problema olhamos para o data type do ExpAr.
p.e no  primeiro caso é Left pois é a primeira coisa que aparece no data type e nao precisamos de colocar nada dentro do either para o determinar.
No lado esquerdo encontra se a expressão e no direito a sua derivada.
\begin{code}
 
sd_gen :: Floating a =>
    Either () (Either a (Either (BinOp, ((ExpAr a, ExpAr a), (ExpAr a, ExpAr a))) (UnOp, (ExpAr a, ExpAr a)))) -> (ExpAr a, ExpAr a)
sd_gen (Left ()) = (X, N 1) 
sd_gen (Right (Left k)) = (N k,N 0) 
sd_gen (Right (Right (Left (Sum, ((d1, d2),(d3, d4))))))= (Bin Sum d1 d3,Bin Sum d2 d4)
sd_gen (Right (Right (Left (Product, ((d1, d2),(d3, d4))))))= (Bin Product d1 d3,(Bin Sum (Bin Product d1 d4) (Bin Product d2 d3)))
sd_gen (Right (Right (Right (Negate, (d1, d2))))) = (Un Negate d1, Un Negate d2)
sd_gen(Right (Right (Right (E, (d1,d2))))) = (Un E d1, Bin Product (Un E d1) d2 )


\end{code}
Para resolver este problema começamos por ver o tipo da função ad_gen e o raciocínio foi semelhante ao anterior.
\begin{code}
ad_gen:: Floating a =>a-> Either()(Either a(Either(BinOp, ((ExpAr a, a), (ExpAr a, a))) (UnOp, (ExpAr a, a)))) -> (ExpAr a, a)
ad_gen _ (Left ()) = (X, 1) 
ad_gen _ (Right (Left k)) = (N k, 0) 
ad_gen _ (Right (Right (Left (Sum, ((d1, d2),(d3, d4))))))= (Bin Sum d1 d3,d2+d4)
ad_gen x (Right (Right (Left (Product, ((d1, d2),(d3, d4))))))  = (Bin Product d1 d3, d4 * eval_exp x d1  + d2 * eval_exp x d3)
ad_gen _ (Right (Right (Right (Negate, (d1, d2))))) = (Un Negate d1,negate d2)
ad_gen x (Right (Right (Right (E, (d1,d2))))) = (Un E d1, (expd (eval_exp x d1))*d2)
...
\subsection*{Problema 2}
A partir de \begin{eqnarray} C_n = \frac{(2n)!}{(n+1)! (n!) } \label{eq:cat}\end{eqnarray} temos que \begin{eqnarray} \frac {C_n}{C_n_+_1} = \frac {\frac{(2n)!}{(n+1)! (n!)}}{\frac{(2(n+1))!}{((n+1)+1)! ((n+1)!)}} \end{eqnarray} Assim, \begin{eqnarray} \frac{C_n}{C_n_+_1} = \frac {(2n)!(n+2)!(n+1)!}{(n+1)!n!(2(n+1))!} = \frac {(2n)!(n+2)!}{n!(2(n+1))!} = \frac {(n+2)!}{2n!(n+1)(2n+1)} = \frac{(n+2)(n+1)}{2(n+1)(2n+1)} = \frac{n+2}{2(2n+1)} \end{eqnarray} Logo, \begin{eqnarray} C_n_+_1 = \frac{2(2n+1)}{n+2}C_n \end{eqnarray} 
Seja $cat\ = c\ n$ a função pretendida. Podemos ver que |c 0 = 1| e, tal como visto em cima, $|c (n+1)| = \frac{2(2n+1)}{n+2} * |c n| = \frac {4n +2}{n+2} * |c n|$. Se definirmos $|a n| = 4n + 2$ e $|b n| = n + 2$ temos:
...
Logo, a solução é da seguinte forma:
\begin{code}
c :: Integer -> Integer
c 0 = 1
c (n+1) = div ((a n) * c n) (b n)
...
calcLine = cataList h where
   h = (either (const(const nil)) x)
   x :: (Rational, NPoint -> OverTime NPoint) -> (NPoint -> OverTime NPoint)
   x (d,f) l = case l of
       []     -> nil
       (x:xs) -> \z -> concat $ (sequenceA [singl . linear1d d x, f xs]) z
